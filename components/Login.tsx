...
export const Login: React.FC<LoginProps> = ({ onLoginSuccess }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            if (isRegistering) {
                // Criar nova conta com trial de 48h
                if (!name.trim()) {
            ...
            <div className="relative z-10 w-full max-w-md bg-studio-card border border-studio-accent p-8 rounded-3xl shadow-2xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <button 
                        type="submit"
                        disabled={loading}
                        className="w-full py-4 rounded-xl font-bold text-lg bg-gradient-to-r from-neon-purple to-neon-pink text-white hover:shadow-[0_0_20px_rgba(189,0,255,0.4)] transition-all transform hover:scale-[1.02] mt-4 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                    >
                        {loading ? (
