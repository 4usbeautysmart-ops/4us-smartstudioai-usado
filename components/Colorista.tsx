...
export const Colorista: React.FC = () => {
  ...
  const generatePDFDoc = () => {
      ...
      doc.text(`Dossiê Colorista: ${clientName}`, pageWidth / 2, cursorY, { align: "center" });
      doc.setFontSize(12);
      doc.text(`Marca: ${brand}`, pageWidth / 2, cursorY + 15, { align: "center" });
      cursorY += 40;
      ...
      try {
        ...
        if (clientImage) doc.addImage(clientImage, 'PNG', margin, cursorY, imgSize, imgSize, undefined, 'FAST');
        if (editedImage) doc.addImage(editedImage, 'PNG', margin + imgSize + 10, cursorY, imgSize, imgSize, undefined, 'FAST');
      ...
      if (report.formula.toner) formulaText += `\n\nTonalizante: ${report.formula.toner}`;
      if (report.formula.alternatives) formulaText += `\n\nAlternativa: ${report.formula.alternatives}`;
      ...
      addSection("Cuidados Pós-Química (Home Care)", report.postChemicalCare, true);
  ...
  const handleSharePDF = async () => {
      ...
      try {
          ...
          const file = new File([blob], `4us_colorista_${clientName.replace(/\s+/g, '_')}.pdf`, { type: "application/pdf" });
  ...
  const handleImageCompositeAction = async (action: 'save' | 'share') => {
    ...
    // Images with labels
    const drawImageWithLabel = (img: HTMLImageElement, x: number, y: number, w: number, h: number, label: string) => {
    ...
    drawImageWithLabel(imgBefore, PADDING, PADDING + TITLE_HEIGHT, targetWidth, heightBefore, "Antes");
    drawImageWithLabel(imgAfter, PADDING * 2 + targetWidth, PADDING + TITLE_HEIGHT, targetWidth, heightAfter, "Depois");
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 pb-10">
      <div className="absolute top-8 right-8 z-20">
          <button onClick={() => handleCloseReport()} className="text-gray-500 hover:text-white transition-colors">
                    ...
                    {editedImage ? (
                        ...
                        <div className="flex w-full h-full">
                            ...
                            <div className="w-1/2 relative bg-black flex items-center justify-center overflow-hidden">
                                <img src={clientImage!} className="w-full h-full object-contain opacity-80" alt="Antes" />
                                <span className="absolute bottom-2 left-2 bg-black/50 text-white text-xs px-2 py-1 rounded">Antes</span>
                            </div>
                            <div className="w-1/2 relative bg-black flex items-center justify-center overflow-hidden">
                                <img src={editedImage} className="w-full h-full object-contain" alt="Depois" />
                                <span className="absolute bottom-2 right-2 bg-purple-600 text-white text-xs px-2 py-1 rounded">Depois</span>
                            </div>
                        </div>
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity z-10">
                            <button onClick={() => handleImageCompositeAction('save')} className="bg-black/60 backdrop-blur-sm text-white px-4 py-2 rounded-full flex items-center gap-2 border border-white/20 hover:bg-black/80"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>Salvar Imagem</button>
                            <button onClick={() => handleImageCompositeAction('share')} className="bg-black/60 backdrop-blur-sm text-white px-4 py-2 rounded-full flex items-center gap-2 border border-white/20 hover:bg-black/80"><svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M15 8a3 3 0 10-2.977-2.63l-4.94 2.47a3 3 0 100 4.319l4.94 2.47a3 3 0 10.895-1.789l-4.94-2.47a3.027 3.027 0 000-.74l4.94-2.47C13.456 7.68 14.19 8 15 8z" /></svg>Compartilhar</button>
                        </div>
                        </>
                    ...
                    <div className="bg-studio-card rounded-2xl border border-studio-accent p-6 space-y-4 animate-in fade-in duration-500">
                      <div className="flex justify-between items-center border-b border-gray-700 pb-4 flex-wrap gap-4">
                          ...
                          <div className="flex gap-2">
                              ...
                              <button onClick={handleCloseReport} title="Fechar" className="text-gray-500 hover:text-red-500 ml-2">
