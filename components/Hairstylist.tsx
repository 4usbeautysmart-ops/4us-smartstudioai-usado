...
import {
  generateHairstylistConsultancy,
  editHairImage,
...
export const Hairstylist: React.FC = () => {
  ...
  const handleProcess = async () => {
    if (!clientImage || !clientName.trim() || (!prompt.trim() && !refImage)) {
      alert(
        "Preencha o nome da cliente e forneça a foto da cliente e uma descrição ou foto de referência."
      );
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      const reportData = await generateHairstylistConsultancy(
        clientBase64,
        clientMime,
        prompt,
        brand,
      ...
      if (reportData.prompts) {
        ...
        try {
          ...
          const compressedGenerated = await Promise.all(
            Object.values(images).map((img) =>
              compressImageForStorage(img!, 1024)
            )
          );
  ...
  const handleShareImage = async (
    ...
    try {
      ...
      const file = new File(
        [blob],
        `4us_hairstylist_${clientName}_${view}.png`,
        { type: blob.type }
      );
  ...
  const generatePDFDoc = () => {
    ...
    doc.text(`Plano de Corte 5D: ${clientName}`, pageWidth / 2, cursorY, {
      align: "center",
    });
    cursorY += 40;
    ...
    try {
      ...
      clientImage &&
        doc.addImage(clientImage, "PNG", margin, cursorY, imgSize, imgSize);
      ...
      if (cursorY + imgSize > pageHeight - margin) {
        doc.addPage();
        cursorY = margin;
      }
    ...
    addSection(
      ...
      report.toolsAndAccessories.join(", ")
    );
    addSection("Diagrama 3D (Descrição)", report.diagram3d);
    addSection(
      "Passo a Passo Técnico",
      report.techniqueStepByStep.map((s, i) => `${i + 1}. ${s}`)
    );
  ...
  const handleSharePDF = async () => {
    ...
    try {
      ...
      const file = new File(
        [blob],
        `plano_corte_5d_${clientName.replace(/\s+/g, "_")}.pdf`,
    ...
    <div className="grid lg:grid-cols-2 gap-8 h-full">
      <div className="space-y-6">
        <div className="bg-studio-card p-6 rounded-2xl border border-studio-accent/50 shadow-lg">
          <h2 className="text-3xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-neon-cyan to-purple-400">
            Hairstylist Visagista 5D
          </h2>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-studio-bg border border-gray-700 rounded-lg p-3 text-white outline-none"
              ...
              <div className="relative group h-64 border-2 border-dashed border-gray-600 rounded-xl p-4 flex justify-center items-center bg-black/30">
                {!clientImage && (
                  <input
                    type="file"
                    accept="image/*"
                    onChange={handleFileChange(setClientImage)}
                    className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10"
                  />
                )}
                {clientImage ? (
                  <img src={clientImage} className="max-h-full rounded-lg" />
                ) : (
                  <p className="text-gray-400">Arraste ou clique</p>
                )}
              </div>
              {!clientImage && (
                <div className="grid grid-cols-2 gap-4 mt-2">
                  <button
                    ...
                    className="py-2 rounded-lg font-bold text-white bg-gradient-to-r from-purple-600 to-blue-600 shadow-lg text-sm"
                  >
                    Selfie
                  </button>
                  <button
                    onClick={() => startCamera("environment")}
                    className="py-2 rounded-lg font-bold text-gray-300 bg-studio-card border border-gray-600 text-sm"
                  >
                    Câmera Traseira
                  </button>
                </div>
              ...
              <div className="h-48 border-2 border-dashed border-gray-600 rounded-xl p-4 flex justify-center items-center bg-black/30 relative">
                {!refImage && (
                  <input
                    type="file"
                    accept="image/*"
                    onChange={handleFileChange(setRefImage)}
                    className="absolute inset-0 w-full h-full opacity-0 z-10"
                  />
                )}
                {refImage ? (
                  <img src={refImage} className="max-h-full rounded-lg" />
                ) : (
                  <p className="text-gray-400">Enviar Referência</p>
                )}
              </div>
              {refImage && (
                <button
            ...
            <textarea
              ...
              placeholder="Corte Desejado (Opcional com Referência)"
              className="w-full h-24 bg-studio-bg border border-gray-700 rounded-lg p-3 text-white"
            />

            <select
              value={brand}
              onChange={(e) => setBrand(e.target.value)}
              className="w-full bg-studio-bg border border-gray-700 rounded-lg p-3 text-white"
            >
              {BRANDS.map((b) => (
          ...
          <button
            ...
            className="w-full mt-6 py-4 rounded-xl font-bold bg-gradient-to-r from-neon-cyan to-blue-600 text-white disabled:opacity-50"
          >
          ...
          {report ? (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1 relative group">
                  <img
                    src={clientImage!}
                    className="w-full aspect-square object-contain bg-black/30 rounded-lg"
                  />
                  <p className="text-center text-xs text-gray-400">Antes</p>
                </div>
                ...
                {(["front", "side", "back"] as const).map((view) => (
                  <div key={view} className="space-y-1 relative group">
                    ...
                    <p className="text-center text-xs text-gray-400">
                      Depois ({view.charAt(0).toUpperCase() + view.slice(1)})
                    </p>
                    {generatedImages[view] && (
                      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() =>
                            handleShareImage(generatedImages[view], view)
                          }
                          className="bg-black/60 backdrop-blur-sm text-white px-3 py-1 text-xs rounded-full border border-white/20"
                        >
                          Compartilhar
                        </button>
                      </div>
              ...
              <ReportSection title="Segredos de Finalização">
                <ul className="list-disc list-inside">
                  {report.finalizationSecrets?.map((tip, i) => (
                    <li key={i}>{tip}</li>
                  ))}
                </ul>
              </ReportSection>
            ...
            <div className="h-full flex items-center justify-center text-gray-600">
              {loading ? (
                ...
                <p>O plano de corte aparecerá aqui.</p>
              )}
            </div>
          )}
        </div>
