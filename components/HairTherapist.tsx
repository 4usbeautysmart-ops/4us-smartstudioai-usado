...
export const HairTherapist: React.FC<HairTherapistProps> = ({ onNavigate }) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const result = await generateHairTherapyConsultancy(
        base64Data,
        mimeType,
        description,
        brand
  ...
  const generatePDFDoc = () => {
    ...
    doc.text(
      `Protocolo de Terapia Capilar: ${clientName}`,
      pageWidth / 2,
      cursorY,
      { align: "center" }
    );
    ...
    addSection(
      ...
      report.treatmentPlan.stepByStep.map((s, i) => `${i + 1}. ${s}`)
    );
    addSection(
      "Cronograma Capilar (4 Semanas)",
      Array.isArray(report.treatmentPlan.schedule)
        ? report.treatmentPlan.schedule.join("\n")
  ...
  const handleSavePDF = () => {
    ...
    if (doc) doc.save(`terapia_${clientName.replace(/\s+/g, "_")}.pdf`);
  };
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File(
        [blob],
        `terapia_capilar_${clientName.replace(/\s+/g, "_")}.pdf`,
  ...
  const ReportSection: React.FC<{
    ...
    colorClass?: string;
  }> = ({ title, children, colorClass = "text-cyan-300" }) => (
    <div>
      <strong className={`block ${colorClass} mb-2`}>{title}:</strong>
      <div className="text-gray-300 space-y-1 text-sm">{children}</div>
    </div>
  );
    ...
    <div className="grid lg:grid-cols-2 gap-8 h-full">
      ...
      <div className="space-y-6">
        <div className="bg-studio-card p-6 rounded-2xl border border-studio-accent/50 shadow-lg relative">
          ...
          <h2 className="text-3xl font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-500">
            Terapeuta Capilar
          </h2>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="relative group h-48 border-2 border-dashed border-gray-600 rounded-xl p-4 flex items-center justify-center bg-black/30 hover:border-red-500 transition-colors">
                ...
                {image ? (
                  ...
                  <p className="text-gray-400">Enviar foto</p>
                )}
              </div>
              {!image && (
            ...
            <textarea
              ...
              placeholder="Queixa do cliente (ex: Cabelo quebradiço, sem brilho)"
              className="w-full h-24 bg-studio-bg border border-gray-700 rounded-lg p-3 text-white outline-none"
            />

            <button
              onClick={handleAnalyze}
              disabled={!image || !description || !clientName || loading}
              className="w-full py-4 rounded-xl font-bold text-lg bg-gradient-to-r from-red-600 to-orange-600 text-white disabled:opacity-50"
            >
      ...
      <div className="bg-studio-card p-6 rounded-2xl border border-studio-accent/50 shadow-lg flex flex-col h-full">
        <h3 className="text-xl font-bold mb-4 text-white shrink-0">
          Prontuário: {clientName}
        </h3>
            ...
            <div className="space-y-6 animate-in fade-in duration-500">
              <div className="bg-red-900/20 p-6 rounded-2xl border border-red-500/30">
                ...
                <div className="space-y-4 text-sm">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                      {report.diagnosis.damageLevel}
                    </ReportSection>
                    <ReportSection title="Porosidade" colorClass="text-red-300">
                      {report.diagnosis.porosity}
                    </ReportSection>
                    <ReportSection
                      title="Elasticidade"
                      colorClass="text-red-300"
                    >
                      {report.diagnosis.elasticity}
                    </ReportSection>
                    <ReportSection
                      title="Couro Cabeludo"
                      colorClass="text-red-300"
                    >
                      {report.diagnosis.scalpCondition}
                    </ReportSection>
                  </div>
                  <ReportSection
                    title="Resumo do Especialista"
                    colorClass="text-red-300"
                  >
              ...
              <div className="bg-studio-accent/50 p-6 rounded-2xl border border-gray-700">
                <h4 className="text-xl font-bold text-neon-cyan mb-2">
                  {report.treatmentPlan.protocolName}
                </h4>
                <p className="text-sm text-gray-400 mb-4">
                  Protocolo de tratamento com{" "}
                ...
                <div className="space-y-4">
                  <ReportSection title="Produtos Recomendados">
                    <ul className="list-disc list-inside space-y-1">
                      {report.treatmentPlan.products.map((p, i) => (
                        <li key={i}>{p}</li>
                      ))}
                    </ul>
                  </ReportSection>
                  <ReportSection title="Passo a Passo da Aplicação">
                    <ol className="list-decimal list-inside space-y-1">
                      {report.treatmentPlan.stepByStep.map((s, i) => (
                        <li key={i}>{s}</li>
                      ))}
                    </ol>
                  </ReportSection>
                  ...
                  {report.treatmentPlan.lifestyleTips && (
                    <ReportSection title="Dicas Adicionais (Estilo de Vida)">
                      <ul className="list-disc list-inside space-y-1">
                        {report.treatmentPlan.lifestyleTips.map((tip, i) => (
                          <li key={i}>{tip}</li>
                        ))}
                      </ul>
                    </ReportSection>
            ...
            >
              ...
              >
                <path
                  ...
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 G- 01-15.357-2m15.357 2H15"
                />
              </svg>
            </button>
          </div>
