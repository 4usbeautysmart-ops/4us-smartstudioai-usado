      ...
      <div className="space-y-6">
        <div className="bg-studio-card p-6 rounded-2xl border border-studio-accent/50 shadow-lg">
            ...
            <div className={`
              ...
              ${loading || generatingCuts ? 'border-neon-violet/80' : 'border-gray-600'}
              ${!image && !loading && !generatingCuts ? 'hover:border-neon-cyan cursor-pointer' : ''}
            `}>
                {/* Scanner Effect */}
                {(loading || generatingCuts) && (
                ...
                {!image ? (
                    ...
                    <img src={image} alt="Preview" className={`max-h-64 mx-auto rounded-lg shadow-lg object-cover group-hover:scale-105 transition-transform duration-500 ${loading || generatingCuts ? 'opacity-80 scale-105' : ''}`} />
                )}
            </div>
            
            {(loading || generatingCuts) && (
                <div className="mt-4 text-center">
                    <p className="text-neon-cyan font-mono text-sm animate-pulse">{loadingMessage}</p>
                </div>
            )}
            ...
            >
                {loading || generatingCuts ? 'Processando...' : 'Iniciar Consultoria'}
            </button>
      ...
      <div className="bg-studio-card p-6 rounded-2xl border border-studio-accent/50 shadow-lg overflow-hidden flex flex-col h-[600px]">
        ...
        <h3 className="text-xl font-bold mb-4 text-neon-cyan flex items-center justify-between shrink-0">
            ...
            <div className="flex items-center gap-2">
                 {analysisData && (
                    <>
                        <button onClick={handleReset} title="Nova Análise" className="text-gray-400 hover:text-neon-cyan transition-colors p-2 bg-studio-bg rounded-lg border border-gray-700 flex items-center gap-2">
                                        ...
                                        <div className="relative group w-full h-64 bg-black/50 rounded-lg overflow-hidden flex items-center justify-center border border-gray-800">
                                            {cut.generatedImage ? (
                                                ...
                                                <div className="flex flex-col items-center justify-center text-neon-cyan">
                                                    ...
                                                    <span className="text-xs animate-pulse">Gerando simulação...</span>
                                                </div>
                                            )}
                                        </div>
                                        {cut.generatedImage && (
